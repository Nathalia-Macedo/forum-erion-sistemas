...
const CategoryCard = ({ category }) => {
  ...
  const handleCreatorClick = (e) => {
    ...
    if (user && user.idUsuario === category.criadoPor.idUsuario) {
      navigate('/user');
    } else {
      navigate(`/user/${category.criadoPor.idUsuario}`);
    }
  };
    ...
    >
      ...
      <div className="latest-post flex items-center gap-3 w-full sm:w-1/3">
        ...
        <div className="post-info overflow-hidden">
          <p className="post-title text-sm font-medium text-gray-700 mb-1 truncate">{category.subTitulo}</p>
          <p 
            className="post-meta text-xs text-gray-500 truncate hover:text-blue-500 transition-colors duration-200"
